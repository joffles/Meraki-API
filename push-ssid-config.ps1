...
# This script sets the wifi configuration for a single site
...
# Define splash page config
    ...
    # Push SSID/Splash Config. Note: Try/catch routines are needed to capture errors
        # Penny
            Try {
                # SSID Config
                    $response = Invoke-RestMethod "https://api.meraki.com/api/v1/networks/$($networkid)/wireless/ssids/0" -Method 'PUT' -Headers $headers -Body $pennyssid
                    write-output $response

                # Splash Config
                    $response = Invoke-RestMethod "https://api.meraki.com/api/v1/networks/$($networkid)/wireless/ssids/0/splash/settings" -Method 'PUT' -Headers $headers -Body $pennysplash -Verbose
